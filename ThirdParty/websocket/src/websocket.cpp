...
int32_t parse_websocket_request(const char *s_req, ws_req_t *ws_req) {
	...
	if (p) {
		...
		while (p = strtok(NULL, delim)) {
			...
			if ((q = strstr(p, ":")) != NULL) {
				*q = '\0';
				if (_stricmp(p, "Connection") == 0) {
					while (*++q == ' ');
					ws_req->connection = q;
				}
				if (_stricmp(p, "Upgrade") == 0) {
					while (*++q == ' ');
					ws_req->upgrade = q;
				}
				if (_stricmp(p, "Host") == 0) {
					while (*++q == ' ');
					ws_req->host = q + 1;
				}
				if (_stricmp(p, "Origin") == 0) {
					while (*++q == ' ');
					ws_req->origin = q;
				}
				if (_stricmp(p, "Cookie") == 0) {
					while (*++q == ' ');
					ws_req->cookie = q;
				}
				if (_stricmp(p, "Sec-WebSocket-Key") == 0) {
					while (*++q == ' ');
					ws_req->sec_websocket_key = q;
				}
				if (_stricmp(p, "Sec-WebSocket-Version") == 0) {
					while (*++q == ' ');
...
string generate_key(const string &key) {
	...
	SHA1 sha;
	sha.addBytes(tmp.c_str(), tmp.length());

	//base64 encode
	unsigned char * digest = sha.getDigest();
	string res = base64_encode(digest, 20);
	delete [] digest;
