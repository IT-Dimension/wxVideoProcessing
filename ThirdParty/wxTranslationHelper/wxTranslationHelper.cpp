...
wxTranslationHelper::wxTranslationHelper(const wxString & search_path,
										 bool use_native_config)
: m_SearchPath(search_path), m_ConfigPath(wxEmptyString), 
m_Locale(NULL), m_UseNativeConfig(use_native_config)
{	
	if(search_path.IsEmpty())
	{
		m_SearchPath = wxPathOnly(wxTheApp->argv[0]);
	}
}
...
bool wxTranslationHelper::Load()
{
	...
	{
		config = new wxConfig(wxTheApp->GetAppName());
	}
	else
	{
		config = new wxFileConfig(wxTheApp->GetAppName(), wxEmptyString, m_ConfigPath);
	}
	long language;
												...
												wxArrayLong & identifiers)
{
	...
	{
		wxLogTrace(_("Directory %s DOES NOT EXIST !!!"),
											 ...
											 wxArrayLong & identifiers)
{
	...
	if(index != -1)
	{
		...
		m_Locale->AddCatalogLookupPathPrefix(m_SearchPath);
		wxLogTrace(_("wxTranslationHelper: Path Prefix = \"%s\""), 
			m_SearchPath.GetData());
		m_Locale->AddCatalog(wxTheApp->GetAppName());
		wxLogTrace(_("wxTranslationHelper: Catalog Name = \"%s\""), 
			wxTheApp->GetAppName().GetData());
		return true;
	}
	return false;
}
