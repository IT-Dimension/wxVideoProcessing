...
wxXmlNode* xsSerializable::SerializeObject(wxXmlNode* node)
{
	if(!node || (node->GetName() != wxT("object")))
	{
		...
		node->AddAttribute(new wxXmlAttribute(wxT("type"), this->GetClassInfo()->GetClassName()));
	}
...
void wxXmlSerializer::SetRootItem(xsSerializable* root, bool bDeleteCurrentRoot /*= true*/)
{
    //wxASSERT(root);
    //wxASSERT(root->IsKindOf(CLASSINFO(xsSerializable)));

	if (m_pRoot && bDeleteCurrentRoot)
		delete m_pRoot;
...
bool wxXmlSerializer::SerializeToXml(const wxString& file, bool withroot)
{
	...
	if(outstream.IsOk())
	{
		...
		wxMessageBox(wxT("Unable to initialize output file stream."), m_sOwner, wxOK|wxICON_ERROR);
...
bool wxXmlSerializer::SerializeToXml(wxOutputStream& outstream, bool withroot)
{
	...
	if(root)
	{
		...
		{
			wxMessageBox(wxT("Unable to save XML document."), m_sOwner, wxOK|wxICON_ERROR);
			return false;
		}
	}
...
bool wxXmlSerializer::DeserializeFromXml(const wxString& file)
{
	...
	if(instream.IsOk())
	{
		...
		wxMessageBox(wxT("Unable to initialize input stream."), m_sOwner, wxOK|wxICON_ERROR);
	...
	try
	{
		...
		if(root && (root->GetName() == m_sRootName))
		{
		    ...
		    if( (owner == m_sOwner) && (version == m_sVersion) )
		    {
                ...
                wxMessageBox(wxT("No matching file owner or version."), m_sOwner, wxOK|wxICON_WARNING);
		}
		else
			wxMessageBox(wxT("Unknown file format."), m_sOwner, wxOK|wxICON_WARNING);
	}
	catch (...)
	{
		wxMessageBox(wxT("Unable to load XML file."), m_sOwner, wxOK|wxICON_ERROR);
	}
